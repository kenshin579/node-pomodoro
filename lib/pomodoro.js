...
let pomoPath = process.cwd() + '/.pomodoro';

try {
    fs.statSync(pomoPath);
} catch (err) {
    pomoPath = process.env.HOME + "/.pomodoro"
}
...
Pomodoro.prototype.run = (message) => {
    ...
    var running = setInterval(function () {
        ...
        if (now >= startTime + TWENTYFIVE) {
            ...
            notifier.notify({
                message: `${message} pomodoro is complete`,
                icon: imageLocation,
                wait: true
            })
...
Pomodoro.prototype.show = function () {
    fs.readFile(pomoPath, "utf-8", function (err, data) {
        if (err) {
            fs.open(pomoPath, "w", function (err, fd) {
                if (err) throw err;
                fs.write(fd, JSON.stringify(items), null, "utf-8", function (err, written, buffer) {
                    if (err) throw err;
                    console.log('Created:', pomoPath);
...
Pomodoro.prototype.add = function (message) {
    fs.readFile(pomoPath, "utf-8", function (err, data) {
        if (err) {
            ...
            items.todo.push(message);
            fs.unlink(pomoPath, function (err) {
                fs.writeFile(pomoPath, JSON.stringify(items), "utf-8", function (err) {
                    if (err) throw err;
                });
            });
        }
